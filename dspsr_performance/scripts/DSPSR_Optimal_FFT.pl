...
sub usage()
{
  ...
  print "   -numa <node>          Bind computation and memory to numa node [default 0]\n";
...
my $cmd = "dada_db -k aaaa -b $block_size -l -n 256 -p -c 0 > /dev/null 2>&1";
if ($verbose)
{
  print $cmd."\n";
}
...
while (!$mem_bad)
{
  ...
  my $junk_thread = threads->new(\&junkThread, "aaaa", $mibytes_per_second, $header, $numa);
  
  sleep 1;

  my $cmd = "numactl -N $numa dspsr";
  if ($overlap)
  {
    $cmd .= " -overlap";
  }

  $cmd .= " -r -D $dm -cuda $gpu -minram 4000";
...
sub junkThread($$$$)
{
  my ($key, $rate, $header, $node) = @_;

  my $ten_rate = 10.0 * $rate;
  my $cmd = "dada_junkdb -g -k $key -R $ten_rate -z -t 1 $header";
  if ($verbose)
  {
    print $cmd."\n";
  }
  `numactl -N $node $cmd > /dev/null 2>&1`;
